...
from PyQt5.QtWidgets import QCalendarWidget, QVBoxLayout, QDialog, QPushButton, QApplication
import sys
...
def fetch_data(ticker, start_date, interval):
    ...
    data = yf.download(ticker, start=str_start_date, interval=interval)
    if data.empty:
...
class CalendarDialog(QDialog):
    def __init__(self, max_days):
        ...
        self.calendar.setMinimumDate(datetime.now() - timedelta(days=max_days))
        self.calendar.setMaximumDate(datetime.now())
...
def get_date(prompt, max_days):
    app = QApplication(sys.argv)
    dialog = CalendarDialog(max_days)
    dialog.exec()
...
def main():
    ...
    num_steps = int(T / dt)
    t, S = geometric_brownian_motion(S0, mu, sigma, T, dt, num_steps, end_datetime, interval)
    simulated_data = pd.DataFrame({'Time': t,'Simulated Price': S})
    ...
    plt.plot(data['25th Percentile'], label='75th Percentile', linestyle='--', color='orange')
    plt.plot(data['50th Percentile'], label='50th Percentile', linestyle='--', color='green')
    plt.plot(data['75th Percentile'], linestyle='--', color='orange')
    ...
    plt.plot(simulated_data, label='Simulated Prices', linestyle='--', color='red')
...
if __name__ == "__main__":
    return_m, return_y, volatility_m,volatility_y,df_simulations = main()
